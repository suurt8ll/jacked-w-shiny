...
def migrate_data(csv_path, input_db_path, output_db_path):
    ...
    try:
        ...
        # 4. Read CSV and insert/replace data
        ...
        with open(csv_path, 'r', newline='', encoding='utf-8') as infile:
            ...
            for i, row in enumerate(reader):
                try:
                    row_id = safe_int(row.get('id'), default=None)
                    if row_id is None:
                        print(f"Warning: Skipping row {i+1} due to missing or invalid 'id'.")
                        skipped_rows += 1
                        continue
                    ...
                    try:
                        ...
                        datetime.fromisoformat(created_str.replace(' ', 'T'))
                         ...
                         print(f"Warning: Skipping row {i+1} (ID: {row_id}) due to invalid 'created' date format: '{created_str}'. Expected ISO format like YYYY-MM-DDTHH:MM:SS.")
                         skipped_rows += 1
                         continue
                    ...
                    if name.lower() == 'weight':
                        ...
                        sql = """
                            INSERT OR REPLACE INTO weights (id, value, created, unit)
                            VALUES (?, ?, ?, ?);
                        """
                        ...
                        # SQLite will handle conversion based on the column's actual affinity
                        params = (
                            row_id,
                        ...
                        sql = """
                            ...
                            (id, name, reps, weight, created, unit, hidden, image, sets, minutes, seconds, steps)
                            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?);
                        """
                    ...
                    print(f"Warning: Skipping row {i+1} due to missing column: {e}")
                    skipped_rows += 1
                except Exception as e:
                    print(f"Warning: Skipping row {i+1} (ID: {row.get('id', 'N/A')}) due to unexpected error: {e}")
                    skipped_rows += 1
        ...
        print(f"Inserted/Replaced in 'sets': {inserted_sets}")
        print(f"Inserted/Replaced in 'weights': {inserted_weights}")
